...
// DMBBootstrapBuilder.csproj FocusSpotEffect_ImageExtension.cs create at 2026/04/21
...
namespace DMBEffectBuilder
{
    ...
    {
        /// <summary>Overlays a radial spotlight vignette on the image that lightens on hover while gently scaling the image up, drawing focus to the centre.</summary>
        ...
        /// <param name="hoverOpacity">The reduced opacity of the overlay on hover, revealing more of the image (0.0 to 1.0). Defaults to 0.18.</param>
        ...
        /// <param name="unit">The unit of measurement for <paramref name="spotSize"/>. Defaults to <see cref="UnitSize.percent"/>.</param>
        /// <returns>The modified <see cref="ImageRenderBuilder"/> instance with the effect applied.</returns>
        /// <remarks>
        /// <para><b>Use cases:</b> Product spotlights, portrait thumbnails, and featured-content cards where guiding the viewer's eye to the centre of the image on hover improves perceived interactivity.</para>
        /// <para><b>How it works:</b> Sets CSS variables <c>--eb-image-effect-opacity</c>, <c>--eb-image-effect-hover-opacity</c>, <c>--eb-image-effect-scale</c>, and <c>--eb-image-effect-spot-size</c>, then adds the class <c>eb-image-effect-focus-spot</c> to the media wrapper via <c>FocusSpotEffect.css</c>.</para>
        /// <para><b>Combinations:</b> Pairs naturally with <c>DarkenToNormalEffect</c> for a load-then-hover two-stage reveal. Avoid combining with <c>GlassRevealEffect</c> as both introduce overlay layers that will visually compete.</para>
        /// <para><b>Tips:</b> Increase <paramref name="spotSize"/> for wide, subject-heavy images and decrease it for portrait-oriented images with a centred subject. Keep <paramref name="scale"/> close to 1.0 (1.01–1.04) to avoid layout shifts on small containers.</para>
        /// <para><b>Example:</b>
        /// <code>
        ...
        /// </remarks>
        [Documented]
        public static ImageRenderBuilder FocusSpotEffect(this ImageRenderBuilder builder, double opacity = 0.55, double hoverOpacity = 0.18, double scale = 1.025, int spotSize = 28, UnitSize unit = UnitSize.percent)
        {

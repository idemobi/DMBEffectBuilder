...
// DMBBootstrapBuilder.csproj LiftHoverEffect_ImageExtension.cs create at 2026/04/21
...
namespace DMBEffectBuilder
{
    ...
    /// This extension is particularly useful for enhancing user experience by providing visual feedback on interactive elements
    ...
    public static class LiftHoverEffect_ImageExtension
    {
        ...
        /// <remarks>
        /// <para><b>Use cases:</b> Clickable image cards, product thumbnails, team member photos, and any image inside a link that benefits from a tangible "pick up" cue. The shadow grounds the lifted image and makes the interaction feel physical.</para>
        /// <para><b>How it works:</b> Sets CSS variables <c>--eb-image-effect-lift-distance</c>, <c>--eb-image-effect-scale</c>, and <c>--eb-image-effect-shadow-opacity</c> on the image element, then adds the class <c>eb-image-effect-lift-hover</c> to the media wrapper via <c>LiftHoverEffect.css</c>. The CSS applies a <c>transform: translateY()</c> combined with <c>box-shadow</c> on <c>:hover</c>.</para>
        /// <para><b>Combinations:</b> Pairs very well with <c>GlowHoverEffect</c> — the lift separates the image spatially while the glow frames it. Also combines cleanly with <c>GrayscaleToColorEffect</c> for a dual-cue hover. Avoid pairing with <c>PolaroidEffect</c> since the Polaroid's own shadow and border visually interfere with the lift shadow.</para>
        /// <para><b>Tips:</b> A <paramref name="distance"/> of 4–8 px feels natural for small card images; use 10–16 px for larger hero-sized images. Keep <paramref name="shadowOpacity"/> between 0.10 and 0.25 — lower values look barely lifted, higher values produce a heavy shadow that competes with the image. Only set <paramref name="scale"/> above 1.0 if you also need the image to grow; a value of 1.03–1.05 combined with a lift reads as a confident "pop".</para>
        /// <para><b>Example:</b>
        /// <code>

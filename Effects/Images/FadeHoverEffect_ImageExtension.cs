...
// DMBBootstrapBuilder.csproj FadeHoverEffect_ImageExtension.cs create at 2026/04/21
...
namespace DMBEffectBuilder
{
    ...
    {
        /// <summary>Reduces the image opacity to a configurable level when the user hovers over it, producing a smooth fade interaction.</summary>
        ...
        /// <remarks>
        /// <para><b>Use cases:</b> Image grids, product listings, and navigation thumbnails where a gentle opacity dip on hover signals interactivity without distracting from the content.</para>
        /// <para><b>How it works:</b> Sets the CSS variable <c>--eb-image-effect-opacity</c> and adds the class <c>eb-image-effect-fade-hover</c> to the media wrapper via <c>FadeHoverEffect.css</c>. Opacity transitions smoothly on hover in and out.</para>
        /// <para><b>Combinations:</b> Combines well with <c>FocusSpotEffect</c> or <c>GlassRevealEffect</c> for a layered hover response. Avoid combining with <c>GhostEffect</c> as both set the same opacity variable and will conflict.</para>
        /// <para><b>Tips:</b> Values between 0.5 and 0.8 give a natural fade. Values below 0.4 make the image nearly invisible on hover, which may confuse users expecting a highlight rather than a disappear effect.</para>
        /// <para><b>Example:</b>
        /// <code>

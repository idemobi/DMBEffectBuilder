...
namespace DMBEffectBuilder
{
    ...
    public static class RotateRandomEffect_ImageExtension
    {
        ...
        /// <param name="hoverDeg">
        ///     Fixed rotation applied on hover, overriding the random angle (default 0.0 — returns to
        ///     straight).
        /// </param>
        /// <returns>The modified <see cref="ImageRenderBuilder" /> instance with the effect applied.</returns>
        /// <remarks>
        ///     <para>
        ///         <b>Use cases:</b> Photo galleries, scrapbook-style layouts, polaroid grids, or any design where a "casually
        ///         scattered" aesthetic is desired. Each server-side render picks a new angle so repeated page loads vary the
        ///         composition naturally.
        ///     </para>
        ///     <para>
        ///         <b>How it works:</b> Picks a random <c>double</c> between <paramref name="fromDeg" /> and
        ///         <paramref name="toDeg" /> using <c>Random.Shared</c> at render time, writes it to the CSS variable
        ///         <c>--eb-rotate-random-angle</c> on the wrapper element, and adds class <c>eb-image-effect-rotate-random</c>
        ///         (RotateRandomEffect.css). Scale and hover values are stored in matching CSS variables on the same wrapper.
        ///     </para>
        ///     <para>
        ///         <b>Combinations:</b> Pairs naturally with <c>SoftShadowEffect</c> to reinforce the physical-print illusion.
        ///         Avoid combining with <c>RotateEffect</c> (spinning animation) or <c>TiltParallaxEffect</c> — the competing
        ///         transforms will fight each other.
        ///     </para>
        ///     <para>
        ///         <b>Tips:</b> Keep the range narrow (±3–6 deg) for a realistic scattered-photos look. Use a wider range
        ///         (±15–30 deg) for a playful, chaotic feel. Set <c>hoverDeg: 0</c> and <c>hoverScale: 1.05</c> to let the user
        ///         "straighten and lift" the photo on hover — a satisfying micro-interaction.
        ///     </para>
        ///     <para>
        ///         <b>Example:</b>
        ///         <code>
        ...
        /// </remarks>
        [Documented]
        public static ImageRenderBuilder RotateRandomEffect(
            this ImageRenderBuilder builder,
            double fromDeg = -4d,
            double toDeg = 4d,
            double scale = 1.0d,
            double hoverScale = 1.0d,
            double hoverDeg = 0.0d
        )
        {

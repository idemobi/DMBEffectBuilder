...
// DMBBootstrapBuilder.csproj ColorInvertEffect_ImageExtension.cs create at 2026/04/21
...
namespace DMBEffectBuilder
{
    ...
    public static class ColorInvertEffect_ImageExtension
    {
        ...
        /// <param name="amount">The percentage of color inversion applied on hover, from 0 (none) to 100 (full inversion). Defaults to 100.</param>
        /// <returns>The modified <see cref="ImageRenderBuilder"/> instance with the effect applied.</returns>
        /// <remarks>
        /// <para><b>Use cases:</b> Dark-themed UIs, artistic galleries, or interactive cards where a striking negative-film look on hover adds visual contrast.</para>
        /// <para><b>How it works:</b> Sets the CSS variable <c>--eb-color-invert-amount</c> as a percentage and adds the class <c>eb-image-effect-color-invert</c> to the media wrapper via <c>ColorInvertEffect.css</c>. The inversion activates on hover.</para>
        /// <para><b>Combinations:</b> Combines well with <c>FadeHoverEffect</c> for a layered hover response. Avoid pairing with <c>ColorFlickerEffect</c> since both manipulate color rendering and will conflict.</para>
        /// <para><b>Tips:</b> A value of 100 gives a clean full inversion; values around 50 produce a muted, desaturated look. Use partial inversion (60–80) for a subtler artistic effect rather than a sharp negative.</para>
        /// <para><b>Example:</b>
        /// <code>

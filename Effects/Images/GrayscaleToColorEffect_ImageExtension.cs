...
// DMBBootstrapBuilder.csproj GrayscaleToColorEffect_ImageExtension.cs create at 2026/04/21
...
namespace DMBEffectBuilder
{
    ...
    /// a valid HttpContext to function correctly. It is recommended to use this method in conjunction with other PageBuilder
    ...
    public static class GrayscaleToColorEffect_ImageExtension
    {
        ...
        /// <param name="amount">The grayscale intensity in the resting (non-hovered) state (0.0 = full color, 1.0 = fully desaturated). Default is 1.0.</param>
        ...
        /// <remarks>
        /// <para><b>Use cases:</b> Gallery grids and card listings where most images should read as a neutral backdrop until the user actively explores one — the color reveal is a clear affordance that the image is interactive. Also effective for "before/after" demonstrations or historical photo sections.</para>
        /// <para><b>How it works:</b> Sets CSS variables <c>--eb-image-effect-grayscale</c> (resting state) and <c>--eb-image-effect-hover-grayscale</c> (hover state), then adds the class <c>eb-image-effect-grayscale-to-color</c> to the media wrapper via <c>GrayscaleToColorEffect.css</c>. The CSS applies a smooth <c>filter: grayscale()</c> transition between both variables.</para>
        /// <para><b>Combinations:</b> Stacks well with <c>LiftHoverEffect</c> — the color reveal and the physical lift reinforce each other as a compound hover cue. Do not combine with <c>GrayscaleHoverEffect</c> or <c>RainbowEffect</c> since all three drive the same <c>filter</c> property.</para>
        /// <para><b>Tips:</b> Keep <paramref name="hoverAmount"/> at 0.0 for maximum contrast. If you want a partial reveal effect (color but still slightly muted), try <paramref name="hoverAmount"/> = 0.2–0.3. Setting both parameters to the same value produces no visible transition and should be avoided.</para>
        /// <para><b>Example:</b>
        /// <code>

...
// DMBBootstrapBuilder.csproj ColorFlickerEffect_ImageExtension.cs create at 2026/04/21
...
namespace DMBEffectBuilder
{
    ...
    {
        /// <summary>Animates the image in a continuous loop, alternating between full color and grayscale to create a flickering color effect.</summary>
        ...
        /// <remarks>
        /// <para><b>Use cases:</b> Retro or neon-style imagery, attention-grabbing hero images, gaming or sci-fi themed UIs where a looping animated energy feel is desired.</para>
        /// <para><b>How it works:</b> Sets the CSS variable <c>--eb-color-flicker-size</c> and adds the class <c>eb-image-effect-color-flicker</c> to the media wrapper via <c>ColorFlickerEffect.css</c>. The animation runs continuously without user interaction.</para>
        /// <para><b>Combinations:</b> Pairs well with <c>GlitchEffect</c> for a distressed look. Avoid combining with <c>GhostEffect</c> or <c>FadeHoverEffect</c> as competing opacity/color changes will clash visually.</para>
        /// <para><b>Tips:</b> Keep <paramref name="size"/> between 2 and 6px for a subtle flicker; larger values produce a more aggressive displacement. Use sparingly — constant animation can distract users on content-heavy pages.</para>
        /// <para><b>Example:</b>
        /// <code>

...
// DMBBootstrapBuilder.csproj TiltParallaxEffect_ImageExtension.cs create at 2026/04/21
...
namespace DMBEffectBuilder
{
    ...
    {
        /// <summary>Applies a 3D tilt transform to the image on hover, creating a parallax depth illusion driven by CSS perspective.</summary>
        /// <param name="builder">The <see cref="ImageRenderBuilder"/> to apply the effect to.</param>
        /// <param name="scale">Scale factor applied on hover to subtly grow the image as it tilts (default 1.03 — a barely perceptible lift).</param>
        ...
        /// <remarks>
        /// <para><b>Use cases:</b> Hero images, feature cards, product showcases — anywhere a sense of physical depth makes the UI feel interactive. Works especially well on large images where the 3D perspective is visible. Also effective for framed or book-cover style images.</para>
        /// <para><b>How it works:</b> Sets CSS variables <c>--eb-image-effect-scale</c>, <c>--eb-image-effect-rotate-x</c>, <c>--eb-image-effect-rotate-y</c>, and <c>--eb-image-effect-translate-z</c> on the image element, then wraps it in a media component with class <c>eb-image-effect-tilt-parallax</c> (TiltParallaxEffect.css). The effect relies on CSS <c>perspective</c> and <c>transform</c> on hover — no JavaScript is used.</para>
        /// <para><b>Combinations:</b> Works well on its own or with <c>SoftShadowEffect</c> — the shadow gets "cast" as the image tilts, reinforcing the 3D illusion. Avoid combining with <c>RotateEffect</c> or <c>ShakeEffect</c> — conflicting transforms will cancel or distort the parallax.</para>
        /// <para><b>Tips:</b> The defaults produce a right-leaning tilt; swap the sign of <c>rotateY</c> to lean left. Keep <c>rotateX</c> and <c>rotateY</c> between -10 and 10 degrees for a realistic effect — beyond that the perspective distortion becomes exaggerated. Increase <c>translateZ</c> to 30–40 px for a more dramatic pop-out on larger images. The effect requires <c>perspective</c> to be set on a parent element in the stylesheet — confirm TiltParallaxEffect.css sets it on the wrapper.</para>
        /// <para><b>Example:</b>
        /// <code>

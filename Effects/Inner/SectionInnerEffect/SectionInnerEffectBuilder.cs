...
// DMBEffectBuilder.csproj SectionInnerBuilder.cs create at 2026/04/13
...
namespace DMBEffectBuilder
{
    ...
    public sealed class SectionInnerEffectBuilder : HtmlTagBuilder<SectionInnerEffectBuilder>, IDisposable, ICanUseCustomClasses, ICanUseMargin
    {
        ...
        /// <remarks>
        /// <para><b>Use cases:</b> call when you need a structured header area (title, subtitle, buttons) above or overlaying the section body.</para>
        /// <para><b>How it works:</b> returns a new <see cref="SectionInnerEffectTitle"/> whose <c>Build</c> method serialises the header HTML and passes it back to this builder via <c>WriteHeader</c>. The header is rendered either before the body div (default) or inside an overlay wrapper when <see cref="SetOverlay"/> is active.</para>
        /// <para><b>Combinations:</b> call <see cref="SetOverlay"/> before <see cref="Begin"/> to change header placement; the title builder chain must end with <c>Build()</c> to register the header HTML.</para>
        /// <para><b>Tips:</b> the title builder is one-shot — creating a second instance overwrites the first header; configure everything in a single chain before calling <c>Build()</c>.</para>
        /// <para><b>Example:</b>
        /// <code>
        ...
        /// <summary>Switches the title header to overlay mode, positioning it on top of the section body content instead of above it.</summary>
        /// <returns>The current builder instance for chaining.</returns>
        /// <remarks>
        /// <para><b>Use cases:</b> use when the section body contains a full-bleed background image or visual element and the title should float over it rather than push it down.</para>
        /// <para><b>How it works:</b> adds the CSS class <c>eb-section-inner-overlay</c> to the container and causes <c>Begin</c> to render the header inside a <c>.eb-section-inner-header-overlay</c> div that is positioned absolutely over the body div.</para>
        /// <para><b>Combinations:</b> call before <see cref="Begin"/>; has no effect after the container has been opened. Pairs well with a full-height body that contains an image or a parallax component.</para>
        /// <para><b>Tips:</b> ensure the body content has enough height to give the overlaid title room to breathe; consider a semi-transparent background on the title for legibility over busy visuals.</para>
        /// <para><b>Example:</b>
        /// <code>
        /// Html.SectionInnerEffectBuilder()

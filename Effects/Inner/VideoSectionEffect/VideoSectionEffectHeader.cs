...
// DMBBootstrapBuilder.csproj VideoSectionEffectHeader.cs create at 2026/04/15
...
namespace DMBEffectBuilder
{
    ...
    /// Set the title text, optional subtitle, optional button, then call <see cref="Build"/> to return to the parent builder.
    /// </summary>
    [Documented]
    public sealed class VideoSectionEffectHeader : TitleBuilderBase<VideoSectionEffectBuilder, VideoSectionEffectHeader>
    {
        ...
        /// <remarks>
        /// <para><b>Use cases:</b> add a short supporting sentence that elaborates on the main title — a tagline, date, or location line.</para>
        /// <para><b>How it works:</b> HTML-encodes the value via <c>HtmlEncoder.Default</c> and wraps it in a <c>&lt;p class="eb-video-section-subtitle"&gt;</c> element appended after the title markup.</para>
        /// <para><b>Combinations:</b> rendered between the title (from the base class) and the button (from <see cref="SetButton"/>); omit entirely if no subtitle is needed — the element is not emitted when the value is empty.</para>
        /// <para><b>Tips:</b> keep subtitle text concise (one sentence); longer text may overflow on small viewports when the video section has a fixed height.</para>
        /// <para><b>Example:</b>
        /// <code>
        ...
        /// <param name="variant">Bootstrap variant style controlling the button colour. Default: <see cref="VariantStyle.Primary"/>.</param>
        /// <returns>The current builder instance for chaining.</returns>
        /// <remarks>
